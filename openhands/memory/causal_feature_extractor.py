...
def _compute_action_hash(action: Action) -> str:
    ...
    if isinstance(action, CmdRunAction):
        ...
        return hashlib.md5(
            f'{action.path}:{action.content[:100]}'.encode()
        ).hexdigest()[:8]
...
def _compute_repetition_score(
    current_hash: str, recent_hashes: list[str], window: int = 10
) -> float:
...
def extract_features(
    events: list[Event], max_events: int = 100
    ...
    for event in events:
        ...
        if isinstance(event, Action):
            ...
            feature_vector[5] = _compute_repetition_score(
